...
rna_gff <- ape::read.gff('GTDB/Outputs_Complete/combined_rrna.gff', GFF3 = T) %>%
...
operons <- operons %>%
  select(-attributes) %>%
  mutate(OperonID = paste0('C_', str_pad(rep(1:(length(seqid)/2), each = 2), width = 8, pad = 0))) %>%
  ...
  left_join(read.delim('GTDB/Outputs_Complete/seq_length.tsv', header = F, sep = '\t'),
            by = c('seqid' = 'V1')) %>%
...
operons %>%
  ...
  write.table('GTDB/Outputs_Complete/operons_unlinked_or_crossingboundary.tsv', quote = F, row.names = F, sep = '\t')
...
operons %>%
  write.table('GTDB/Outputs_Complete/master_rrna.gff', quote = F, row.names = F, sep = '\t')
...
operons %>%
  ...
  write.table('GTDB/Outputs_Complete/full_ITS.gff', quote = F, row.names = F, col.names = F, sep = '\t')
...
operons %>%
  ...
  write.table('GTDB/Outputs_Complete/full_operons.gff', quote = F, row.names = F, col.names = F, sep = '\t')

# create a file with the unique operon identifiers that can be substituted into the fasta file downstream
operons %>%
  mutate(seqid_operon = paste0(seqid, ' ', OperonID)) %>%
  select(seqid_operon) %>%
  write.table('GTDB/Outputs_Complete/operon_identifiers.txt', row.names = F, col.names = F, quote = F, sep = '\t')
