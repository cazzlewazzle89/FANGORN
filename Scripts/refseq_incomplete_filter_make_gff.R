...
rna_gff <- ape::read.gff('RefSeq/Outputs_Incomplete/combined_rrna.gff', GFF3 = T) %>%
  ...
  filter(attributes %in% c('16S ribosomal RNA', '23S ribosomal RNA', '16S Ribosomal RNA', '23S Ribosomal RNA', 'ribosomal RNA-16S', 'ribosomal RNA-23S')) %>%
...
operons <- operons %>%
  ...
  left_join(read.delim('RefSeq/Outputs_Incomplete/seq_length.tsv', header = F, sep = '\t')) %>%
...
operons %>%
  ...
  write.table('RefSeq/Outputs_Incomplete/operons_unlinked_or_crossingboundary.tsv', quote = F, row.names = F, sep = '\t')
...
operons %>%
  write.table('RefSeq/Outputs_Incomplete/master_rrna.gff', quote = F, row.names = F, sep = '\t')
...
operons %>%
  ...
  write.table('RefSeq/Outputs_Incomplete/full_ITS.gff', quote = F, row.names = F, col.names = F, sep = '\t')
...
operons %>%
  ...
  write.table('RefSeq/Outputs_Incomplete/full_operons.gff', quote = F, row.names = F, col.names = F, sep = '\t')

# create a file with the unique operon identifiers that can be substituted into the fasta file downstream
operons %>%
  mutate(seqid_operon = paste0(seqid, ' ', OperonID)) %>%
  select(seqid_operon) %>%
  write.table('RefSeq/Outputs_Incomplete/operon_identifiers.txt', row.names = F, col.names = F, quote = F, sep = '\t')
